...
df = pd.concat([train_ratings_df, test_ratings_df], axis=0)
df = pd.merge(df, users_df[user_features], on='user_id', how='left')
...
# 학습 및 테스트 데이터 분리
train_df = df_label[df_label['rating'] != 0].copy()  # rating이 0이 아닌 데이터는 학습 데이터로 사용
test_df = df_label[df_label['rating'] == 0].copy()   # rating이 0인 데이터는 테스트 데이터로 사용

train_X = train_df.drop(columns=['rating'])
train_y = train_df['rating']
test_X = test_df.drop(columns=['rating'])
test_y = test_df['rating']

# 추론 시 사용되지 않음print('***** FM & FFM 데이터 *****')
...
# Test 데이터 예측 및 저장
def predict_and_save(test_dataloader, model, device, sample_submission_df):
    model.eval()
    predictions = []
    ...
    with torch.no_grad():
        for X, _ in tqdm(test_dataloader, desc="Predicting"):
            X = X.to(device)
            preds = model(X)
            # 예측값을 1에서 10 사이의 정수로 변환 후 리스트에 추가
            preds = torch.clamp(preds.round(), min=1, max=10).int()  # 반올림과 클램핑
            predictions.extend(preds.cpu().numpy())

    # 예측 결과를 sample_submission 형식에 맞게 저장
    sample_submission_df['rating'] = predictions
    submission_path = 'FFM_submission.csv'
    sample_submission_df.to_csv(submission_path, index=False)
    print(f"Predictions saved to {submission_path}")

# 예측 함수 호출
predict_and_save(test_dataloader, model, device, sample_submission_df)
