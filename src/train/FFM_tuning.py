...
class FFMLayer(nn.Module):
    def __init__(self, field_dims, factor_dim):
        ...
        ----------
        >>> x = torch.Tensor([[1, 0, 0], [2, 1, 0], [0, 0, 1]])
        >>> ffm = FFMLayer(field_dims=[3, 3, 3], factor_dim=2)
        >>> ffm(x)
        '''
        super().__init__()
    ...
    def forward(self, x):
        ...
        xv = [self.embedding[f](x)              # FILL HERE : fill in the places `None` using `self.embedding` #
              for f in range(self.num_fields)]  # FILL HERE : fill in the places `None` using `self.embedding` #

        y = list()
        for f in range(self.num_fields):
            for g in range(f + 1, self.num_fields):
                y.append(xv[f][:, g] *  xv[g][:, f])
        y = torch.stack(y, dim=1)  # (batch_size, num_fields * (num_fields - 1) / 2)
...
class FieldAwareFM(nn.Module):
    def __init__(self, field_dims, factor_dim):
        ...
        super().__init__()
        self.feature_dim = sum(field_dims)
        self.linear = FeaturesLinear(field_dims, 1, bias=True) # FILL HERE : Fill in the places `None` #
    ...
    def forward(self, x):
        ...
        Parameter
            x: Long tensor of size "(batch_size, num_fields)"
                x_multihot: Multi-hot coding of x. size "(batch_size, self.feature_dim)"
        ...
        y = self.linear(x).squeeze(1) + self.ffm(x) # FILL HERE : Use `self.linear()` and `self.ffm()` #
...
factorization_dims = [4, 8, 16]
learning_rates = [0.0001, 0.001]
weight_decays = [1e-4, 1e-5]
epochs = 20  # Epoch 수를 고정합니다.
task = 'reg'
...
# 결과를 기록할 로그 파일 경로
log_path = 'FFM_hyperparameter_tuning_log.txt'

# 결과를 로그 파일에 기록하는 함수
def log_results(params, train_loss, test_err, test_loss):
    with open(log_path, 'a') as log_file:
        log_file.write(f"Parameters: {params}\n")
        log_file.write("Epoch | Train Loss | Test Error | Test Loss\n")
        for epoch, (tr_loss, tst_err, tst_loss) in enumerate(zip(train_loss, test_err, test_loss), 1):
            log_file.write(f"{epoch:02d}    | {tr_loss:.6f}  | {tst_err:.6f}   | {tst_loss:.6f}\n")
        log_file.write("====================================\n")

# 하이퍼파라미터 튜닝 실행
for batch_size in batch_sizes:
    for factorization_dim in factorization_dims:
        for learning_rate in learning_rates:
            for weight_decay in weight_decays:
                print(f"\nTraining with batch_size={batch_size}, factorization_dim={factorization_dim}, "
                ...
                # 모델 훈련 및 평가
                train_loss, test_err, test_loss = train_and_test(
                    train_dataloader, test_dataloader, model, loss_fn, optimizer, epochs, task, device
                )

                # 결과 기록
                params = {
                    'batch_size': batch_size,
                    'factorization_dim': factorization_dim,
                    'learning_rate': learning_rate,
                    'weight_decay': weight_decay,
                    'epochs': epochs
                }
                log_results(params, train_loss, test_err, test_loss)